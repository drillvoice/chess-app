...
- **Fix deleted sessions coming back**: the same cause with a worse effect. The local record was removed outright, so a delete whose cloud write was skipped had nothing left to retry from — and the next cloud snapshot, which still held the session, silently restored it. A delete now leaves a marker behind that the next sync pass turns into a cloud tombstone, and the marker clears itself once the cloud agrees. Sessions deleted on another device also no longer reappear when an account first syncs onto a new device
- **Fix reviewed Lichess games returning to the review queue**: each device tracked which games it had imported in its own browser storage, and imported games carried no identity of their own, so the same game imported on two devices became two unrelated entries. A device whose tracker had fallen behind re-imported everything played since — as new games needing review, dated at the original games — and cloud sync then spread the duplicates to every device. Imports are now matched against the games already in your log, so a game imported anywhere is skipped rather than duplicated, and a device that had fallen behind catches its tracker up quietly
- **Archive all**: when more than three games are waiting for review, the card offers a one-click "Archive all (N)". Archived games stay in your log with everything recorded on them — they simply stop needing review. This is also the quickest way to clear out duplicates the import bug above already created, since the fix stops new ones rather than removing existing ones
- Fix a mistake tag created on one device not showing up in the tag picker on another until the page was reloaded: the vocabulary reached local storage, but the picker kept showing the list as it stood when the tab was opened
- Opening repertoires had the same "an edit never leaves the device" defect as sessions, and are fixed the same way
...
- Fix saved tags **disappearing**: the Log game modal could show "No mistake tags yet" for someone who had already created tags, and study tags could quietly revert to the seeded three. A Firestore timestamp lost its type on the way into local storage, which made the whole preferences record fail validation on the next read — so it was replaced with defaults, and the empty list could then sync out to your other devices. A bad timestamp now costs the timestamp, not the tags: the record is repaired on read instead of discarded, tag lists are merged across devices rather than last-writer-wins, and an already-emptied vocabulary is rebuilt once per app load from the tags on your logged sessions (only when the list looks reset, so tags you deleted on purpose stay deleted)
- Adding a tag now tells you when it **fails** — a rejected name or a failed save used to be swallowed silently — and validates against the same rule everywhere (the picker and the schema previously disagreed about apostrophes)
